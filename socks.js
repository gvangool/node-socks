    ...
    */
    ...
    Address = {
      read: function(buffer, offset) {
        var atyp = buffer[offset]
        if (atyp === ATYP.IP_V4) {
          return util.format('%s.%s.%s.%s', buffer[offset+1], buffer[offset+2], buffer[offset+3], buffer[offset+4])
        } else if (atyp === ATYP.DNS) {
          var dlen = buffer[offset+1] // domain's length in byte
          return buffer.toString('utf8', offset+2, offset+2+dlen)
        } else if (atyp === ATYP.IP_V6) {
          return buffer.slice(offset+1, offset+1+16)
        }
      },
      ...
      sizeOf: function(buffer, offset) {
        ...
        if (atyp === ATYP.IP_V4) {
          ...
          return buffer[offset+1] + 1 // must add first byte
        } else if (atyp === ATYP.IP_V6) {
          return 16
        }
      }
...
function createSocksServer(cb) {
  ...
  socksServer.on('listening', function() {
    ...
    llog('LISTENING %s:%s', address.address, address.port)
  })
  socksServer.on('connection', function(socket) {
    llog('CONNECTED %s:%s', socket.remoteAddress, socket.remotePort)
...
function handshake(chunk) {
  ...
  if (chunk[0] != SOCKS_VERSION) {
    lerror('handshake: wrong socks version: %d', chunk[0])
    this.end()
  }
...
function handleRequest(chunk) {
  ...
  if (chunk[0] !== SOCKS_VERSION) {
    ...
    lerror('handleRequest: wrong socks version: %d', chunk[0])
    return
  }
  address = Address.read(chunk, offset)
  offset = 3 + Address.sizeOf(chunk, 3) + 1
  port = chunk.readUInt16BE(offset)

  llog('Request: type: %d -- to: %s:%s', chunk[1], address, port)
...
function proxyReady() {
  llog('Indicating to the client that the proxy is ready')
  // creating response
  ...
  resp[2] = 0x00
  this.write(resp)
  llog('Connected to: %s:%d', resp.toString('utf8', 4, resp.length-2), resp.readUInt16BE(resp.length-2))
}
